...
expect.extend({
  async toHaveLanguage(browser: WebdriverIO.Browser, expectedLanguage: string) {
    ...
    if (language === expectedLanguage) {
      return {
        message: () =>
            `expected browser not to have language "${expectedLanguage}"`,
                 pass: true,
      }
    }

    return {
      message: () => `expected browser to have language "${
          expectedLanguage}" not "${language}"`,
               pass: false,
    }
  }
})
...
expect.extend({
  async toHaveAriaLabel(element: WebdriverIO.Element, expectedAriaLabel: any) {
    const $el = await element
    const ariaLabel = await browser.execute(
        (el) => el.ariaLabel, $el as unknown as HTMLElement)
    if (ariaLabel === expectedAriaLabel) {
      return {
        message: () => `expected element with selector "${
            $el.selector}" not to have an aria label "${expectedAriaLabel}"`,
                 pass: true,
      }
    }
    ...
    return {
      message: () => `expected element with selector "${
          $el.selector}" to have an aria label "${expectedAriaLabel}" not "${
          ariaLabel}"`,
               pass: false,
    }
  }
